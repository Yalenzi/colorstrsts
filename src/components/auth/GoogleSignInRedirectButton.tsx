...
interface GoogleSignInRedirectButtonProps {
  lang?: Language;
...
export function GoogleSignInRedirectButton({
  lang = 'ar',
  onError,
  className = '',
  variant = 'outline',
  ...
  children,
  fullWidth = true
}: GoogleSignInRedirectButtonProps) {
  const [loading, setLoading] = useState(false);
  const isRTL = lang === 'ar';

  const texts = {
    signInButton: isRTL ? 'تسجيل الدخول بـ Google' : 'Continue with Google',
    signingIn: isRTL ? 'جاري إعادة التوجيه...' : 'Redirecting...',
