      ...
      try {
        ...
        if (!test) {
          try {
            ...
            console.log(`🔍 Found test in database service: ${testId}`);
          } catch (dbError) {
            console.warn(`⚠️ Test not found in database service: ${testId}`);
          }
        }

        if (!test) {
          console.error(`❌ Test not found anywhere: ${testId}`);
        ...
        const fallbackResult: TestResult = {
          ...
          confidence: 50,
          timestamp: new Date(),
          colorName: lang === 'ar' ? 'لون غير معروف' : 'Unknown Color',
          colorHex: selectedColor,
          substances: [lang === 'ar' ? 'مادة غير محددة' : 'Unidentified Substance']
        };

        setResult(fallbackResult);
        console.warn(`⚠️ Using fallback result for test: ${testId}`);
      } finally {
        setLoading(false);
      }
    };
